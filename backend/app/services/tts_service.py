...
def ensure_audio_is_generated(
    ...
    try:
        ...
        return {"audio_files": [Path(f).name for f in audio_files]}
...
def ensure_hindi_audio_is_generated(
    ...
    def chunk_hindi_text(text: str, max_chunk_length: int = 450) -> List[str]:
        ...
        for sentence in sentences:
            ...
            if (
                current_chunk_grapheme_length + sentence_grapheme_length + 1
                > max_chunk_length
            ):
                if current_chunk:
                ...
                if sentence_grapheme_length > max_chunk_length:
                    ...
                    for word in words:
                        ...
                        if (
                            temp_chunk_grapheme_length + word_grapheme_length + 1
                            > max_chunk_length
                        ):
        ...
        for i, chunk in enumerate(chunks):
            ...
            if chunk_len > max_chunk_length:
                print(
                    f"Warning: Chunk {i} has {chunk_len} graphemes, which exceeds the maximum of {max_chunk_length}"
                )
            ...
            if cleaned_text:
                ...
                for j, chunk in enumerate(hindi_chunks):
                    chunk_path = os.path.join(
                        temp_dir, f"00_title_introduction_chunk_{j:03d}.wav"
                    )
                    print(
                        f"  Processing chunk {j+1}/{len(hindi_chunks)} ({len(chunk)} chars)"
                    )
        ...
        for i, section_name in enumerate(section_order, start=1):
            if section_name in sections_scripts:
                ...
                if cleaned_text:
                    ...
                    hindi_chunks = chunk_hindi_text(cleaned_text)
                    print(
                        f"Processing {len(hindi_chunks)} Hindi chunks for {section_name}"
                    )
                    # Create temporary directory for chunk files
                    temp_dir = os.path.join(output_dir, "temp_chunks")
                    Path(temp_dir).mkdir(exist_ok=True)
                    ...
                    for j, chunk in enumerate(hindi_chunks):
                        chunk_path = os.path.join(
                            temp_dir,
                            f"{i:02d}_{section_name.lower()}_chunk_{j:03d}.wav",
                        )
                        print(
                            f"  Processing chunk {j+1}/{len(hindi_chunks)} ({len(chunk)} chars)"
                        )
                    ...
                    if chunk_files:
                        ...
                        list_file = os.path.join(
                            temp_dir, f"{i:02d}_{section_name.lower()}_list.txt"
                        )
                        with open(list_file, "w") as f:
                            for chunk_file in chunk_files:
                                f.write(f"file '{os.path.abspath(chunk_file)}'\n")
                        ...
                        try:
                            ...
                            print(
                                f"FFmpeg error: {e.stderr.decode() if e.stderr else e}"
                            )
                            # If ffmpeg fails, use the first chunk as fallback
                            if chunk_files:
                                import shutil
                                ...
                                successful_generations += 1
                                print(
                                    f"⚠ {section_name} Hindi audio (fallback): {audio_path}"
                                )
...
def ensure_language_audio_is_generated(
    ...
    def chunk_text_by_language(
        text: str, language: str, max_chunk_length: int = None
    ) -> List[str]:
    ...
    def generate_audio_from_chunks(
        chunks: List[str], language_code, base_filename: str
    ) -> bool:
    ...
    try:
        ...
        if title_intro_script and title_intro_script.strip():
            ...
            if cleaned_text:
                ...
                if generate_audio_from_chunks(
                    chunks, language_code, "00_title_introduction"
                ):
                    successful_generations += 1
        ...
        for i, section_name in enumerate(section_order, start=1):
            if section_name in sections_scripts:
                ...
                if cleaned_text:
                    ...
                    if show_debug:
                        print(
                            f"Processing {len(chunks)} {language} chunks for {section_name}"
                        )

                    if generate_audio_from_chunks(
                        chunks, language_code, f"{i:02d}_{section_name.lower()}"
                    ):
                        successful_generations += 1
...
def test_sarvam_sdk(api_key: str, voice: str = "anushka"):
