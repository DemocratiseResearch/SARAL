...
class SarvamTTS:
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            response = requests.post(
                self.base_url, headers=headers, json=test_data, timeout=30
            )
    ...
    def synthesize_text(
        ...
        try:
            ...
            print(
                f"Using voice: {voice}, target language: {target_language}, sample rate: {sample_rate}"
            )
            ...
            try:
                ...
                try:
                    if isinstance(audio_content, str):
                        ...
                        raise SarvamTTSError(
                            f"Audio content is not string: {type(audio_content)}"
                        )
            ...
            for i, chunk in enumerate(chunks):
                ...
                try:
                    audio_bytes = self.synthesize_text(
                        chunk, target_language, voice, sample_rate
                    )
                    if audio_bytes:
