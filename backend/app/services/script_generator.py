    ...
    if file_path.endswith(".txt"):
        try:
            ...
            for line in lines[1:20]:  # Look in lines 1-20
                ...
                if re.search(
                    r"\babstract\b|\bintroduction\b|\bsection\b", line.lower()
                ):
                    break
...
def generate_all_bullet_points_with_gemini(api_key, sections_scripts):
    ...
    print(
        f"Generating bullet points for {len(sections_scripts)} sections using single prompt"
    )
...
Process all sections in the input and generate bullet points accordingly."""
    ...
    try:
        ...
        for line in bullet_text.split("\n"):
            ...
            if section_name:
                ...
                for standard_name in sections_scripts.keys():
                    if (
                        standard_name.lower() in section_name.lower()
                        or section_name.lower() in standard_name.lower()
                        or standard_name.lower().replace(" ", "")
                ...
                if not section_matched:
                    print(
                        f"Warning: Could not match section '{section_name}' to any standard section"
                    )
        ...
        for section_name in sections_scripts.keys():
            if (
                section_name not in sections_bullets
                or not sections_bullets[section_name]
            ):
                ...
                if script_text and script_text.strip():
                    ...
                    sections_bullets[section_name] = (
                        sentences[:4]
                        if sentences
                        else ["Key information from this section"]
                    )
                else:
                    sections_bullets[section_name] = [
            ...
            print(
                f"Section {section_name}: {len(sections_bullets[section_name])} bullets"
            )
        ...
        if (
            "API" in str(e).upper()
            or "QUOTA" in str(e).upper()
            or "RATE" in str(e).upper()
        ):
            ...
            print(
                "Response parsing error detected. The AI response might not be in expected format."
            )
        ...
        for section_name, script_text in sections_scripts.items():
            try:
                ...
                print(
                    f"Generated {len(sections_bullets[section_name])} fallback bullets for {section_name}"
                )
