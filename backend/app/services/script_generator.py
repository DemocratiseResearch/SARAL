...
def extract_paper_metadata(file_path):
    ...
    # Enhanced TeX file processing
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            content = f.read()
        
        # Extract title - handle both simple and complex formats
        # Pattern 1: \title[short]{full title} - use the full title (second braces)
        title_match = re.search(r'\\title\[([^\]]*)\]\{([^}]+)\}', content)
        if title_match:
            # Use the full title (second capture group)
            title_raw = title_match.group(2).strip()
        else:
            # Pattern 2: Simple \title{title}
            title_match = re.search(r'\\title\{([^}]+)\}', content)
            if title_match:
        ...
        if title_raw:
            # Clean only KNOWN LaTeX formatting commands from title
            title_clean = re.sub(r'\\\\', ' ', title_raw)  # Replace \\ with space
            
            # List of known LaTeX formatting commands to remove entirely
            known_commands = [
                'textbf', 'textit', 'emph', 'textsf', 'texttt', 'textsc',
                'underline', 'textcolor', 'color', 'large', 'Large', 'LARGE',
                'small', 'footnotesize', 'tiny', 'normalsize', 'huge', 'Huge',
                'centering', 'raggedright', 'raggedleft', 'newline', 'noindent'
            ]
            
            # Remove known formatting commands with their braces
            for cmd in known_commands:
                title_clean = re.sub(rf'\\{cmd}\{{([^}}]+)\}}', r'\1', title_clean)
                title_clean = re.sub(rf'\\{cmd}', '', title_clean)
            
            # For unknown commands (like \sailor), remove the backslash but keep the text
            # This converts \sailor to "Sailor" (capitalize first letter)
            def replace_custom_command(match):
                cmd_name = match.group(1)
                # Capitalize first letter of custom commands
                return cmd_name.capitalize()
            
            title_clean = re.sub(r'\\([a-zA-Z]+)(?![a-zA-Z])', replace_custom_command, title_clean)
            
            # Clean up whitespace
            title_clean = re.sub(r'\s+', ' ', title_clean).strip()
            metadata["title"] = title_clean
            print(f"[METADATA] Extracted title: {title_clean}")
        
        # Extract authors - find ALL \author{} commands
        author_matches = re.findall(r'\\author\{([^}]+)\}', content)
        if author_matches:
            # Join all authors with commas
            authors_list = [author.strip() for author in author_matches]
            metadata["authors"] = ", ".join(authors_list)
            print(f"[METADATA] Extracted {len(authors_list)} authors: {metadata['authors']}")
        ...
        print(f"[METADATA ERROR] Error extracting metadata from LaTeX file: {e}")
...
def extract_text_from_file(file_path):
    ...
    # Enhanced TeX file processing - preserve content while removing LaTeX commands
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            content = f.read()
        ...
        # Extract only document content (between \begin{document} and \end{document})
        doc_match = re.search(r'\\begin\{document\}(.*?)\\end\{document\}', content, re.DOTALL)
        if doc_match:
            content = doc_match.group(1)
        
        # Replace LaTeX commands but KEEP their content
        # Pattern: \command{content} or \command[opt]{content} → content
        # This preserves: \textbf{text} → text, \section{Results} → Results
        content = re.sub(r'\\[a-zA-Z]+\*?(?:\[[^\]]*\])?\{([^}]*)\}', r'\1', content)
        
        # Remove standalone LaTeX commands (no braces)
        content = re.sub(r'\\[a-zA-Z]+\*?\s', ' ', content)
        
        # Remove environment markers but keep content
        content = re.sub(r'\\begin\{[^}]+\}', '', content)
        content = re.sub(r'\\end\{[^}]+\}', '', content)
        
        # Clean up remaining braces and special chars
...
#         print("response text from gemini", response.text)
...
AUDIENCE_PROMPTS = {
    "novice": (
        "Generate a 5-slide presentation narration script for a novice audience regarding the topic of this paper."
    ...
    "intermediate": (
        "Generate a 5-slide presentation narration script for professionals in the field of this paper."
    ...
    "expert": (
        "Generate a 5-slide presentation narration script for a peer-expert audience regarding the topic of this paper."
        "Slide 1: Introduction. State the theoretical gap, research hypothesis, and baseline assumptions. Use precise terminology.Describe all of it in spoken natural language"
        "Slide 2: Methodology. Detail the mathematical framework. Describe equations in spoken natural language. Never use mathematical symbols, subscripts, superscripts, or LaTeX notation"
        "Slide 3: Results. Focus on statistical significance, convergence rates, or error analysis. Provide a high-density data summary.Describe all of it in spoken natural language"
...
AUDIENCE_PROMPTS_BULLETS = {
    ...
    "expert": (
        "Constraints: Prioritize data integrity and objective tone. Ensure all LaTeX is formatted for high-resolution display.Provide the bullet points in a way that is easy to understand for a peer-expert audience.Use LaTeX notation ($...$) for equations and mathematical expressions. These will be rendered in a presentation slide."
    ),
}


def _fix_json_backslashes(raw_text: str) -> str:
    """Escape backslashes that are not valid JSON escapes so ``json.loads`` succeeds.

    LLM outputs for expert-level scientific content often contain raw LaTeX
    commands (``\\text``, ``\\alpha``, ``\\nabla``, …) inside JSON string values.
    Python's ``json.loads`` rejects these because ``\\a``, ``\\o``, ``\\m`` etc.
    are not valid JSON escape sequences.

    This function walks the string character-by-character and doubles any
    backslash that is **not** part of a legitimate JSON escape (``\\"``,
    ``\\\\/``, ``\\\\``, ``\\b``, ``\\f``, ``\\n``, ``\\r``, ``\\t``,
    ``\\uXXXX``).  It also distinguishes ``\\t`` (real tab) from ``\\text``
    (LaTeX) by checking whether the next character after ``t`` is a letter.
    For non-LaTeX responses this is effectively a no-op.
    """
    result: list[str] = []
    i = 0
    length = len(raw_text)
    while i < length:
        if raw_text[i] == '\\' and i + 1 < length:
            nxt = raw_text[i + 1]

            # Unambiguous single-char JSON escapes
            if nxt in ('"', '\\', '/'):
                result.append(raw_text[i:i + 2])
                i += 2
                continue

            # \\uXXXX unicode escape
            if nxt == 'u' and i + 5 < length:
                hex_part = raw_text[i + 2:i + 6]
                if all(c in '0123456789abcdefABCDEF' for c in hex_part):
                    result.append(raw_text[i:i + 6])
                    i += 6
                    continue

            # \\b \\f \\n \\r \\t — only a real JSON escape when NOT followed
            # by another letter (\\text, \\nabla, \\tau → LaTeX).
            if nxt in 'bfnrt':
                if i + 2 < length and raw_text[i + 2].isalpha():
                    result.append('\\\\')  # double the backslash (LaTeX)
                    i += 1
                    continue
                else:
                    result.append(raw_text[i:i + 2])  # keep real escape
                    i += 2
                    continue

            # Anything else (\\alpha, \\omega, \\mathcal, …) — not valid JSON
            result.append('\\\\')
            i += 1
        else:
...
@track_performance
def generate_full_script_with_gemini(api_key, input_text, audience_level=None):
    import google.generativeai as genai
    import json
    import re
    ...
    audience_context = ""
    if audience_level and audience_level.lower() in AUDIENCE_PROMPTS:
        audience_context = AUDIENCE_PROMPTS[audience_level.lower()]
...
@track_performance
def generate_all_bullet_points_with_gemini(api_key, sections_scripts, audience_level=None):
    ...
    audience_context = ""
    if audience_level and audience_level.lower() in AUDIENCE_PROMPTS_BULLETS:
        audience_context = AUDIENCE_PROMPTS_BULLETS[audience_level.lower()]
...
for the audience : {audience_context}
...
• Dont use colons in the bullet points
...
Process all sections in the input and generate bullet points accordingly."""
    ...
    try:
        ...
        print(f"Response preview: {bullet_text}")
        ...
        for line in bullet_text.split('\n'):
            ...
            # Format 5: Plain section name (exact match with known sections)
            elif line in sections_scripts:
                section_name = line
                print(f"Found plain section header: '{section_name}'")
...
@track_performance
def split_script_into_sections(script_json):
    """
    Validate and normalize Gemini JSON output.
    ...
    for section in required_sections:
        content = script_json.get(section, "").strip()
        if not content:
            content = f"Content for {section} section needs to be added."
