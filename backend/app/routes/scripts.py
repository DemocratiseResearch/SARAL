...
def resolve_paper_id(paper_id: str) -> str:
    ...
    First checks if it's already a UUID, then searches for arxiv_id in papers.
    """
    # Check if it's already in storage (UUID)
    if paper_id in papers_storage or storage_manager.get_paper(paper_id):
        return paper_id

    # Search for arxiv_id in all papers
    all_papers = storage_manager.get_all_papers()
    for uuid, paper_info in all_papers.items():
        if paper_info.get("metadata", {}).get("arxiv_id") == paper_id:
            logger.info(f"Resolved arxiv ID {paper_id} to UUID {uuid}")
            return uuid

    # Also check in-memory storage
    for uuid, paper_info in papers_storage.items():
        if paper_info.get("metadata", {}).get("arxiv_id") == paper_id:
            logger.info(f"Resolved arxiv ID {paper_id} to UUID {uuid} (from memory)")
            return uuid
...
async def generate_script(paper_id: str, api_keys: dict = Depends(get_api_keys)):
    ...
    if not paper_info:
        ...
        if paper_id_str not in papers_storage:
            logger.error(
                f"Paper ID {paper_id_str} not found in storage. Available IDs: {list(papers_storage.keys())}"
            )
            raise HTTPException(
    ...
    try:
        ...
        if "tex_file_path" in paper_info:
            ...
            logger.error(
                f"No text or tex file path found. Available keys: {available_keys}"
            )
            raise ValueError(
                f"No text or tex file path found in paper info. Available keys: {available_keys}"
            )
        ...
        all_bullet_points = generate_all_bullet_points_with_gemini(
            api_keys["gemini_key"], cleaned_sections
        )
        ...
        sections_scripts_only = {
            k: v["script"] for k, v in sections_with_bullets.items()
        }

        return ScriptResponse(sections_scripts=sections_scripts_only, paper_id=paper_id)
...
async def get_sections_with_bullets(paper_id: str):
    ...
    try:
        ...
        if not script_data or not script_data.get("sections"):
            raise HTTPException(
                status_code=404, detail="Scripts not found for this paper"
            )
...
async def update_sections(paper_id: str, request: ScriptUpdateRequest):
    ...
    try:
        ...
        if not save_scripts_to_file(paper_id, script_data):
            raise HTTPException(
                status_code=500, detail="Failed to save scripts to file"
            )
...
async def assign_image_to_section(
    paper_id: str, section_name: str, image_name: str = None
):
...
async def refresh_sections_data(paper_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error refreshing scripts: {str(e)}"
        )
