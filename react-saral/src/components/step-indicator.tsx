...
import { type WorkflowStep, useWorkflowStore } from "@/stores/workflow-store"
...
export function StepIndicator({ current }: { current: WorkflowStep }) {
  const { setStep, paperId } = useWorkflowStore()
  const currentIdx = ORDER.indexOf(current)

  const handleClick = (step: WorkflowStep, idx: number) => {
    // Can only navigate to completed steps or the current step
    // Upload is always accessible; other steps need a paperId
    if (idx > currentIdx) return
    if (step !== "upload" && !paperId) return
    setStep(step)
  }
  ...
  return (
    <nav className="flex items-center justify-center gap-2 py-6">
      {STEPS.map((step, idx) => {
        ...
        const clickable =
          idx <= currentIdx && (step.key === "upload" || !!paperId)
        return (
